...
$banco		= parametro("banco", FALLBACK_CLAVE_DE_BANCO, MQL_INT);
$limite		= parametro("limite", 0, MQL_FLOAT);
$cuenta		= parametro("cuenta", "");
$tarjeta	= parametro("tarjeta", "");
$tipo		= parametro("tipo", BANCOS_OPERACION_CHEQUE);
...
if($persona > DEFAULT_SOCIO){
	...
	if($xPer->init() == true){
		...
		if($xRel->addRelacionBancaria($banco, $limite,$tipo,$fecha, $cuenta, $tarjeta) == false){
			$error	= true;
