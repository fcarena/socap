...
$jxc = new TinyAjax();
function jsaListarPermisos($usuario, $superior){
	$superior	= setNoMenorQueCero($superior);
	$xPerm		= new cSystemPermissions();
	$rs			= $xPerm->getPermitidos($usuario, $superior);
	$xTab		= new cHTabla();
	...
	$rs			= $xPerm->getNegados($usuario, $superior);
	foreach ($rs as $rw){
		$xItem->setData($rw);
		$xTab->initRow();
		$id		= $xItem->idgeneral_menu()->v();
...
$jxc ->exportFunction('jsaListarPermisos', array('idniveldeusuario', 'idmenusuperior'), "#idmenu" );
