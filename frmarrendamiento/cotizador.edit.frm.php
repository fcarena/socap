...
if($xUser->getEsOriginador() == true){
	...
	if($xOrg->initByIDUsuario($xUser->getID()) == true){
		...
		$suborigen	= $xOr4g->getSubOriginador();
...
function jsaGetResidual($precio, $aliado, $plazo, $residuales){
	...
	if($NRes <= 0){
		...
		foreach ($rs as $rw){
			$xEsc->setData($rw);
			$idx	= $xEsc->plazo()->v();
			$res 	= $xEmul->getValorResidual($precio, $aliado, $idx);
		...
		foreach ($DRes as $idx => $IRes){
			...
			$res 	= $xEmul->getValorResidual($precio, $aliado, $PRes, $TRes);
...
$jxc ->exportFunction('jsaGetResidual', array('precio_vehiculo','monto_aliado', 'plazo', 'residuales'));
	...
	if($xTabla->credito()->v() > DEFAULT_CREDITO){
		...
		if($xCred->init() == true){
			...
			if($EsOriginador == false){
				...
				if($xArr->init() == true){
					if($xArr->getClaveDeVehiculo() >0){
						//$xFRM->addAvisoRegistroError("No se puede duplicar el vehiculo");
						//$xFRM->addCerrar("", 5);
					} else {
						$xFRM->OButton("TR.AGREGAR DATOS DEL VEHICULO", "jsAgregarDatosVehiculo()", $xFRM->ic()->CREDITO);
					}
				}
			}
...
foreach ($rs as $rw){
	...
	$li		= $D["porciento_residual"];
	$ls		= $D["porciento_final"];
	if($ls <= $li){
		$ls	= $li + 20;
	}
	$arrOpts= array();
	...
	//setLog($xLeas->getTasaResidualPzo($idx));
...
foreach ($rs as $rw){
	$xEsc->setData($rw);$idx	= $xEsc->plazo()->v();
	$tt		.= "<td class='mny' id='total-cuota-$idx'></td>";
}
$tt		.= "</tr>";
...
function jsCalcular(idx){
	...
	//Coste
	var vCoste				= (precio+aliado) - $("#monto_anticipo").val();
	//Opcional
...
function jsAgregarPersona(){
	...
	if(idpersona > DEFAULT_SOCIO){
		...
		//var email				= $("#email").val();
		if(es_moral == 1){
			xP.goToAgregarMorales({nombre:nombres,tipoorigen:vTipoOrigen,claveorigen:idcontrol});
		} else {
			xP.goToAgregarFisicas({nombrecompleto:nombres,tipoorigen:vTipoOrigen,claveorigen:idcontrol});
		}
	}
}
...
function jsAgregarDatosVehiculo(){
	var idcredito	= $("#credito").val();
	var idcontrol	= $("#idoriginacion_leasing").val();
	xG.w({url:"../frmcreditos/creditos-garantias.new.frm.php?tipo=2&credito=" + idcredito + "&tipodeorigen=" +  vTipoOrigen + "&clavedeorigen=" + idcontrol, tiny:true, h:600, w:800});
}
