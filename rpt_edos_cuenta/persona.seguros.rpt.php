...
//=====================================================================================================
$xHP		= new cHPage("TR.REPORTE", HP_REPORT);
$xL			= new cSQLListas();
$xF			= new cFecha();
$xQL		= new MQL();
...
$sql			= "SELECT
	`vw_operaciones_por_annio`.`persona`,
	...
	`vw_operaciones_por_annio`.`ejercicio`,
	`vw_operaciones_por_annio`.`monto` 	
...
FROM
	`socios_general` `socios_general` 
		...
		`idsocios_cajalocal` 
			INNER JOIN `vw_operaciones_por_annio` `vw_operaciones_por_annio` 
			ON `vw_operaciones_por_annio`.`persona` = `socios_general`.`codigo` 
				INNER JOIN `socios_region` `socios_region` 
				ON `socios_general`.`region` = `socios_region`.`idsocios_region`
WHERE
`vw_operaciones_por_annio`.`tipo_de_operacion` = $clave

ORDER BY `vw_operaciones_por_annio`.`ejercicio`, `vw_operaciones_por_annio`.`persona` ";
$titulo			= "";
$archivo		= "";
$xTO			= new cTipoDeOperacion($clave); $xTO->init();
$xHP->setTitle($xHP->getTitle() . " " . $xTO->getNombre());
