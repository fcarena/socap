...
if($credito <= DEFAULT_CREDITO){
	...
	if($xCred->init() == true){
		...
		$sql	= "SELECT `idgeneral_contratos`,`titulo_del_contrato`,`ruta` FROM `general_contratos` WHERE (`tags` LIKE '%" . $xCred->getTipoDeOrigen() .  "%' OR `tags` LIKE '%" . SYS_TODAS .  "%') AND `estatus`='alta' AND `tipo_contrato`=" . iDE_CREDITO;
