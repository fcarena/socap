...
function jsaGetLetrasAVencer($fecha, $producto){
	...
	//$sql2	= $xL->getListadoDeCreditos(false, false, false, "", " AND (DATE_FORMAT(fecha_ministracion, '%d')=DATE_FORMAT('$fecha', '%d')) (`creditos_tipoconvenio`.`tipo_en_sistema` =" . CREDITO_PRODUCTO_INDIVIDUAL . ") ");
	//$sql	= "$sql UNION $sql2";
	$xT		= new cTabla($sql, 2);
	$xT->setWithMetaData();
