...
class cUtileriasParaCreditos{
	...
	function setEstatusDeCreditos($recibo, $fecha = false, $AppSucursal = true, $force_updates = false, $credito = false, $EnCierre = false){
			...
			$ql->setRawQuery("UPDATE creditos_solicitud SET fecha_vencimiento = DATE_ADD(fecha_ministracion, INTERVAL dias_autorizados DAY) WHERE periocidad_de_pago=" . CREDITO_TIPO_PERIOCIDAD_FINAL_DE_PLAZO . " AND saldo_actual >0 AND dias_autorizados >0");
			...
			/** @since 2010-12-27 */
			$sql 			= $xLi->getInicialDeCreditos() . " WHERE (`creditos_solicitud`.`estatus_actual`!=50) $ByCredito AND (`creditos_solicitud`.`saldo_actual`>$tolerancia_en_pesos) ORDER BY `creditos_solicitud`.`fecha_vencimiento` ";
	...
	function setCuadrarCreditosByMvtos($credito = false){
		...
		while($rw2 = $rs2->fetch_assoc() ){
			...
			$xCred		= new cCredito();
			$xCred->init();
			$xCred->setRevisarSaldo(false, $rw);
		}
		$rs2->free();
	...
	function setGenerarMvtoFinDeMes($fecha_inicial, $fecha_final, $NumeroDeCredito = false, $ForzarEnPeriodo = false, $recibo = false ){
		...
		foreach ($rs as $rw){
			...
			$meses			= ceil( $xF->setRestarFechas($fecha_final, $xCred->getFechaDeMinistracion()) / 30.416666666666666666666 );
