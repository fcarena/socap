...
//=====================================================================================================
$xHP		= new cHPage("TR.BUSCAR ACTIVIDAD_ECONOMICA SCIAN", HP_FORM);
$xQL		= new MQL();
$xLi		= new cSQLListas();
$xF			= new cFecha();
...
function jsaGetListadoDeActividades($idnombre){
	$sql	= "SELECT `clave_de_actividad`, `nombre_de_la_actividad` FROM `personas_ae_scian` WHERE `nombre_de_la_actividad` LIKE '%$idnombre%' LIMIT 0,50";
	$xT	= new cTabla($sql);
...
function jsSetIDActividad(id){
	var mObj	= processMetaData("#tr-personas_ae_scian-" + id);
	...
	if(msrc == null){} else {
		if(msrc.getElementById(idfrm)){
			...
			session("ae-scian-" + id, JSON.stringify(mObj));
