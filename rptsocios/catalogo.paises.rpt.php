...
//=====================================================================================================
$xHP		= new cHPage("TR.CATALOGO DE PAIS", HP_REPORT);
$xL			= new cSQLListas();
$xF			= new cFecha();
$xQL		= new MQL();
...
$sql			= "SELECT
	`personas_domicilios_paises`.`clave_de_control`,
	...
	`aml_risk_levels`.`nombre_del_nivel` AS `nivel_de_riesgo` 
FROM
	`personas_domicilios_paises` `personas_domicilios_paises` 
		INNER JOIN `aml_risk_levels` `aml_risk_levels` 
		ON `personas_domicilios_paises`.`es_considerado_riesgo` = 
		`aml_risk_levels`.`clave_de_control`";
$titulo			= "";
$archivo		= "";
