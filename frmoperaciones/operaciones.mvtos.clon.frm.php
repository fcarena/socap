...
if($action == SYS_NINGUNO){
	...
	$xOP->setData( $xOP->query()->initByID($clave) );
	$xFRM->addHElem( $xSel->getListaDeTiposDeOperacion("idtipo", $xOP->tipo_operacion()->v())->get(true) );
	$xFRM->addMonto($xOP->afectacion_real()->v());
	
} else {
	$xOP->setData( $xOP->query()->initByID($clave) );
	$xFol		= new cFolios();
	//$NuevoID	= $xFol->getClaveDeOperaciones();
	$xOP->tipo_operacion($tipo);
	$xOP->afectacion_cobranza($monto);
	$xOP->afectacion_contable($monto);
	$xOP->afectacion_estadistica($monto);
	$xOP->afectacion_real($monto);
	
	$xOP->idoperaciones_mvtos("NULL");
	$success	= $xOP->query()->insert()->save();
