...
//=====================================================================================================
$xHP		= new cHPage("TR.Recibos por Credito", HP_FORM);
$xQL		= new MQL();
$xLi		= new cSQLListas();
$xF			= new cFecha();
...
if($credito <= DEFAULT_CREDITO){
	...
	if($xCred->init() == true){
		...
		$cTblx	= new cTabla($xLi->getListadoDeRecibos("", $xCred->getClaveDePersona(), $xCred->getNumeroDeCredito(), "", "", "", $periodo));
		$cTblx->setKeyField("idoperaciones_recibos");
		$cTblx->setTdClassByType();
		$cTblx->setEventKey("jsGoPanelRecibos");
		$cTblx->OButton("TR.COMPROBANTE", "var xR=new RecGen();xR.formato(" .HP_REPLACE_ID . ")", $xFRM->ic()->IMPRIMIR );
		$xFRM->addHElem( $cTblx->Show());		
	}
}
